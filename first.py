...
def process_single_image_segmentation(image_path, net, visualize=True):
    ...
    if visualize:
        ...
        img_name = os.path.basename(image_path)  # rock_sample_01.png
        name_no_ext = os.path.splitext(img_name)[0]  # rock_sample_01


        ax1.imshow(img_rgb)
        for segment in segments:
            contour = segment['contour']
            ax1.plot(contour[:, 1], contour[:, 0], linewidth=1.5, color='yellow')
        ax1.set_title('Segmented Regions')
        ax1.axis('off')
